...
- **What Fling already has:** a 15-second per-display snapshot that restores on reconnect, layouts with display and wake triggers, and a watcher for new windows.
- **How to build it:**
...
- **License:** Fling's code is original. MIT lets people fork it and use it commercially; GPL (like Loop) keeps forks open. Decide before accepting outside contributions.
...
4. **Later:** CLI and config file (D, full version), always on top via ScreenCaptureKit (F), optional AI arrangements (I).
5. **Before announcing:** notarized builds, a demo GIF of Snap Assist and the grid, and a comparison table against Rectangle Pro, Loop and Stay.
