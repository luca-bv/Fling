...
private struct GeneralSettings: View {
    ...
    @AppStorage(Prefs.fillRest) private var fillRest = true
    @AppStorage(FillRest.Source.shortcut.prefKey) private var assistAfterShortcut = true
    @AppStorage(FillRest.Source.drag.prefKey) private var assistAfterDrag = true
    @AppStorage(FillRest.Source.thrown.prefKey) private var assistAfterThrow = false
    @AppStorage(Prefs.fillRestHold) private var fillRestHold = true
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Toggle("Fill the Rest: after snapping, pick a window to fill the rest", isOn: $fillRest)
                Group {
                    Toggle("…after a keyboard shortcut", isOn: $assistAfterShortcut)
                    Toggle("…and then only while that shortcut's keys are held", isOn: $fillRestHold)
                        .padding(.leading, 18)
                        .disabled(!assistAfterShortcut)
